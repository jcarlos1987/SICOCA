...
Public Class RemocionCOPRCV
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Else
            If Session.Item("SIRECAprintPaeDoc").ToString.Equals("1") Then
                ...
                If Session.Item("SIRECAsubdel").ToString.Equals("01") Then
                    ...
                    LabelSubd20.Text = "Mérida Norte"
                    LabelSubd21.Text = "Mérida Norte"
                ...
                Else
                    ...
                    LabelSubd20.Text = "Mérida Sur"
                    LabelSubd21.Text = "Mérida Sur"
        ...
        If IsDate(Request.QueryString("fecha").ToString) Then
            ...
            LAnio1.Text = Year(fechaMandamiento)
            LAnio2.Text = Year(fechaMandamiento)
        Else
            LFecha.Text = Now.ToLongDateString
            LAnio1.Text = Year(Now)
            LAnio2.Text = Year(Now)
        End If
        labelLeneas.Text = ""
    ...
    Public Sub crearPDF()
        ...
        Dim PDF_FileName As String = Server.MapPath("~") + "/temp/docPae/P" & Request.QueryString("regpat").ToString.Replace("-", "") & "R" & Session.Item("SIRECAid").ToString & Request.QueryString("tipo").ToString & ".pdf"
        '-------------------------------
        ...
        htmlparser.Parse(sr)
        pdfDoc.Close()
        Response.Redirect("../temp/docPae/P" & Request.QueryString("regpat").ToString.Replace("-", "") & "R" & Session.Item("SIRECAid").ToString & Request.QueryString("tipo").ToString & ".pdf")
        'Dim client As WebClient = New WebClient()
        'Dim buffer As Byte() = client.DownloadData(Server.MapPath("~") + "/temp/docPae/Panel" & Request.QueryString("regpat").ToString.Replace("-", "") & "R" & Session.Item("SIRECAid").ToString & ".pdf")
