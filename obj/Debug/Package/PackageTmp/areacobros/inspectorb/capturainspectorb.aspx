<%@ Page Language="vb" MasterPageFile="~/sitios/SiteInspectorB.Master" CodeBehind="capturainspectorb.aspx.vb" Inherits="WebSIRECA.capturainspectorb" %>
...
<asp:Content ID ="Contend1" runat ="server" ContentPlaceHolderID="ContentPlaceHolder1"  >
    <div>
        ...
        <asp:HyperLink ID="HyperLink2" runat="server" 
            NavigateUrl="~/pag_paes/Ejecutor.aspx" Visible="False" >Ir al PAE</asp:HyperLink>
        <asp:SqlDataSource ID="SqlDataSourceFechas" runat="server" 
            ConnectionString="<%$ ConnectionStrings:ConnectionStringMaster %>" 
            ProviderName="<%$ ConnectionStrings:ConnectionStringMaster.ProviderName %>" SelectCommand="SELECT distinct(convert(nvarchar(11),eco.fechaSeleccion)) as fechas
...
and ((year(fechaseleccion)=year(getdate()) and month(fechaseleccion)=month(getdate()))
or (year(fechaseleccion)=year(getdate()) and month(fechaseleccion)=month(getdate())-1)
or (year(fechaseleccion)=2012 and month(fechaseleccion)=12)
)
...
where subdel=@SUBDEL and year(fechaSeleccion)=@ANIO and month(fechaSeleccion)=@MES
and responsable=?
                                                    ...
                                                    <asp:TemplateField HeaderText="REG.PATRONAL" SortExpression="REGISTRO">
                                                        <ItemTemplate>
                                                            <asp:HyperLink ID="HyperLink2" runat="server" 
                                                                ...
                                                                Target="_blank" rel="gb_page_fs[]" Text='<%# Eval("REGISTRO") %>' ToolTip="Observacion al PATRON"></asp:HyperLink>
                                                        </ItemTemplate>
                                                    </asp:TemplateField>
                                                    <asp:TemplateField HeaderText="NOMBRE O RAZON SOCIAL" 
                                                        ...
                                                        <ItemTemplate>
                                                            <asp:HyperLink ID="HyperLink1" runat="server" 
                                                                ...
                                                                title="Zona de Captura"></asp:HyperLink>
                                                        </ItemTemplate>
...
FROM [procesar].[dbo].[RCPRO] as pro inner join sirecacobros.dbo.eco as eco 
on eco.fechaeco=(substring(RCP_PER,6,2)+substring(RCP_PER,1,4)) and [REG#PATRONAL]=[RCP_REGPAT] and eco.tipoEco in ('COP','COMP')
where 
year(RCP_FEC_MOV)=year(DATEADD(month, 1, '1/'+@PERIODO_MES+'/'+@PERIODO_ANIO)) 
and month(RCP_FEC_MOV)=month(DATEADD(month, 1, '1/'+@PERIODO_MES+'/'+@PERIODO_ANIO))
...
inner join sirecacobros.dbo.eco as eco on eco.fechaeco=(substring(RCP_PER,6,2)+substring(RCP_PER,1,4)) and [REG#PATRONAL]=[RCP_REGPAT] and tipoEco in ('RCV','RCVCOMP')
where 
year(RCP_FEC_MOV)=year(DATEADD(month, 1, '1/'+@PERIODO_MES+'/'+@PERIODO_ANIO))
and month(RCP_FEC_MOV)=month(DATEADD(month, 1, '1/'+@PERIODO_MES+'/'+@PERIODO_ANIO))
and RCP_SUB=@SUBDEL and RCP_PER=(@PERIODO_ANIO+'/'+@PERIODO_MES)
...
SET @IMP_DISPONIBLE_COP=(select sum(omision) as IMPORTE_DISPONIBLE from sirecacobros.dbo.eco
where subdel=@SUBDEL 
and fechaEco=(@PERIODO_MES+@PERIODO_ANIO)
and tipoEco in ('COP','COMP')
and eco.responsable=@RESPONSABLE
)
SET @PATRONES_DISPONIBLE_COP=(select count(distinct([REG#PATRONAL])) as IMPORTE_DISPONIBLE from sirecacobros.dbo.eco
where subdel=@SUBDEL 
and fechaEco=(@PERIODO_MES+@PERIODO_ANIO)
and tipoEco in ('COP','COMP')
and eco.responsable=@RESPONSABLE
)
SET @IMP_DISPONIBLE_RCV=(select sum(omision) as IMPORTE_DISPONIBLE from sirecacobros.dbo.eco
where subdel=@SUBDEL 
and fechaEco=(@PERIODO_MES+@PERIODO_ANIO)
                                    ...
                                    <tr>
                                        <td>
                                            <asp:GridView ID="GridView1" runat="server" AllowSorting="True" 
                                                AutoGenerateColumns="False" Caption="ACOMULADO C.O.P." CaptionAlign="Left" 
                                                ...
                                                <Columns>
                                                    ...
                                                    <asp:BoundField DataField="PERIODO" HeaderText="PERIODO" ReadOnly="True" 
                                                        SortExpression="PERIODO" />
                                                    <asp:HyperLinkField DataNavigateUrlFields="URLV" 
                                                        DataNavigateUrlFormatString="../inspectorb/pendientesxinspectorb.aspx{0}" 
                                                        DataTextField="PATRONES" DataTextFormatString="{0:N0}" HeaderText="PATRONES" 
                                                        NavigateUrl="~/areacobros/inspectorb/pendientesxinspectorb.aspx" 
                                                        SortExpression="PATRONES" Target="_blank" />
                                                    <asp:BoundField DataField="CREDITOS" DataFormatString="{0:N0}" 
                                                        HeaderText="CREDITOS" ReadOnly="True" SortExpression="CREDITOS">
                                                    <ItemStyle HorizontalAlign="Right" />
