<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="rep_asignacion2.aspx.vb" Inherits="WebSIRECA.rep_asignacion2" %>
...
<body>
    ...
    <CR:CrystalReportSource ID="CrystalReportSource1" runat="server">
        <Report FileName="rep_asignacionrpt2.rpt">
            <DataSources>
...
DECLARE @FECHA_DILI AS DATETIME
SET @DEL=?
SET @SUBDEL=?
SET @ID_INSPECTOR=?
SET @FECHA_DILI=?
...
,(
isnull(
(SELECT TOP 1 DILI FROM (
SELECT [PC_CODEBAR] AS CODEBAR,'CITATORIO, '+[PC_HORA] AS DILI
,PC_FECHA AS FECH_ACC,PC_FECH_SYSTEM AS FECH_SYS
FROM [sirecacobros].[dbo].[PAE_CITATORIO]
WHERE [PC_INSPECTOR]=[ID_INS] AND [PC_R_ID]=[R_ID]
AND CONVERT(VARCHAR,PC_FECHA,103)=@FECHA_DILI
) AS R1
...
FROM [sirecacobros].[dbo].[PAE_DILIGENCIA]
WHERE [PD_INSPECTOR]=[ID_INS] AND [PD_R_ID]=[R_ID]
AND CONVERT(VARCHAR,PD_FECHA,103)=@FECHA_DILI
) as R2
ORDER BY  FECH_ACC DESC,FECH_SYS DESC)
,'')
)
...
where [R_SUBDEL]=@SUBDEL AND [ID_INS]=@ID_INSPECTOR

) AS X
WHERE FEC_DILIGENCIAX IS NOT NULL
order by REGPAT,convert(datetime,'01/'+[PERIODO],103)
">
        <SelectParameters>
