<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="ceduladecapturaregistro.aspx.vb" Inherits="WebSIRECA.ceduladecapturaregistro1" %>
...
<body>
    ...
    <div>
        ...
        <CR:CrystalReportSource ID="CrystalReportSourceCedulaCaptura" runat="server">
            <Report FileName="cedulaRegistros.rpt">
                <DataSources>
                    <CR:DataSourceRef DataSourceID="SqlDataSourceCedulaCaptura" 
                        TableName="CEDULA_CAPTURA" ReportName="ceduladecapturaregistros.rpt" />
                    <CR:DataSourceRef DataSourceID="SqlDataSourceCedulaCapturaBarra" 
                        ReportName="ceduladecapturabarraregistros.rpt" TableName="CEDULA_CAPTURA" />
                </DataSources>
            </Report>
...
DECLARE @FECHA_CAP_SISCOB as VARCHAR(10)
DECLARE @INC_SISCOB as VARCHAR(6)
DECLARE @DBO AS VARCHAR(50)
SET @SUBDEL=?
SET @FOLIO=?
SET @VINC=?
SET @ANIO=?
SET @FECHA_CAP_SISCOB=?
SET @DBO=?
...
and V_FECHA_P2_TURNADO=@FECHA_CAP_SISCOB)
--TOTAL CREDITOS- - -- - -- - -- - -- - -- - -- - -- - -- - -- - -- - -- - -- - ---
...
,[V_FECHA_P2_TURNADO] as FECHA_CAP_SISCOB
,replace(substring(convert(varchar,V_FECHA_CAPTURA,103),1,5),''/'','''')+substring(convert(varchar,V_FECHA_CAPTURA,103),9,2) as FECHA_DILI
,(
...
,[V_FOLIO_P2_TURNADO_A] as V_INCIDENCIA
,'''' as V_FOLIO_SICOFI
...
,'''+@FECHA_CAP_SISCOB+''' as FECHA_CAP_SISCOB
,''- - - - -'' as FECHA_DILI
...
">
        ...
        <asp:SqlDataSource ID="SqlDataSourceCedulaCapturaBarra" runat="server" 
            ConnectionString="<%$ ConnectionStrings:ConnectionStringMaster %>" 
...
WHERE V_SUBDEL=@SUBDEL
and V_FOLIO_P1=@FOLIO and V_INC_CAPTURA=@VINC and year(V_FECHA_P1_TURNADO)=@ANIO
and V_FOLIO_P2='R'+@FOLIO and V_FECHA_P2_TURNADO=@FECHA_CAP_SISCOB)
--FILAS FALTANTES- - -- - -- - -- - -- - -- - -- - -- - -- - -- - -- - -- - -- - ---
SET @ROWS_MAX=(select case when @NUM_CREDITOS&gt;@ROWS_MAX then @ROWS_MAX-(@NUM_CREDITOS-((@NUM_CREDITOS/@ROWS_MAX)*@ROWS_MAX))
else @ROWS_MAX-@NUM_CREDITOS end)
--ELABORA
...
SELECT
ROW_NUMBER() OVER(order by V_REGPAT,convert(datetime,''01/''+V_PERIODO,103)) as V_N
,'''+@FECHA_CAP_SISCOB+''' as FECHA_CAB_SISCOB
